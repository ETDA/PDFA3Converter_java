...
public class PDFA3Converter {
	...
	private String _inputPDFFilePath, _outputPDFFilePath, _embedFilePath, _colorProfilePath, _metaDataFilePath, _fontFilePath;
	 ...
	 */
	public void setEmbedFile(String embedFilePath, EmbedType embedType) throws Exception {
		
		// Throw FileNotFoundException when input file does not exist
		if (embedFilePath == null) {
			_embedFilePath = null;
		} else {
			if(!(new File(embedFilePath)).exists()) {
				throw new FileNotFoundException("Embed file: " + embedFilePath + " was not found.");
			}
		}
		
		this._embedFilePath = embedFilePath;
