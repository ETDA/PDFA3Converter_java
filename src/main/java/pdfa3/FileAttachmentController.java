...
public class FileAttachmentController {
	 ...
	 * @param inputFileString A URL or path to target file
	 * @return A PDF document with embedded file
	 * @exception IOException Exceptions occur during embed process
	 */
	public PDDocument embedFile(PDDocument document, String inputFileString, EmbedType emdedType) throws IOException {
		oldFileSpecList = new ArrayList<PDComplexFileSpecification>();
		String[] inboundFileNamesList = { inputFileString };
	...
	/*
	 * public PDDocument attachFile(PDDocument document, String inputFileString)
	 * throws Exception {
	 * 
	 * 
	 * PDEmbeddedFilesNameTreeNode embededFileTree = new
	 * PDEmbeddedFilesNameTreeNode();
	 * 
	 * //first create the file specification, which holds the embedded file
	 * PDComplexFileSpecification complexFileSpec = new
	 * PDComplexFileSpecification(); File inputFile = new File(inputFileString);
	 * 
	 * complexFileSpec.setFile(inputFile.getName()); InputStream inputStream = new
	 * FileInputStream(inputFile); PDEmbeddedFile embededFile = new
	 * PDEmbeddedFile(document, inputStream); //set some of the attributes of the
	 * embedded file embededFile.setSubtype("text/xml");
	 * embededFile.setSize((int)inputFile.length()); embededFile.setCreationDate(new
	 * GregorianCalendar()); embededFile.setModDate(new GregorianCalendar());
	 * //embededFile.setFile(complexFileSpec);
	 * 
	 * complexFileSpec.setEmbeddedFile(embededFile);
	 * 
	 * 
	 * complexFileSpec.getCOSObject().setName("AFRelationship", "Alternative");
	 * complexFileSpec.getCOSObject().setString("UF", inputFile.getName());
	 * //complexFileSpec.setFile(inputFile.getName());
	 * 
	 * //now add the entry to the embedded file tree and set in the document.
	 * Map<String, PDComplexFileSpecification> embededFileMap = new HashMap<String,
	 * PDComplexFileSpecification>(); embededFileMap.put("Attachment File List",
	 * complexFileSpec); embededFileTree.setNames(embededFileMap); //attachments are
	 * stored as part of the "names" dictionary in the document catalog
	 * PDDocumentNameDictionary names = new
	 * PDDocumentNameDictionary(document.getDocumentCatalog());
	 * names.setEmbeddedFiles(embededFileTree);
	 * 
	 * PDDocumentCatalog catalog = document.getDocumentCatalog();
	 * catalog.setNames(names);
	 * 
	 * COSArray cosArray = new COSArray(); cosArray.add(complexFileSpec);
	 * catalog.getCOSObject().setItem("AF", cosArray);
	 * 
	 * 
	 * document.getDocumentCatalog().setNames(names);
	 * 
